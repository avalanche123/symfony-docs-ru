...
обернутая фигурными скобками ``{name}``, называется заполнитель) сопоставляется с
...
* *строка 11*: Метод ``render()`` загружает и заполняет шаблон
  (``HelloBundle:Hello:index.twig.html``) переменными, переданными вторым аргументом.
...
Итак, контроллер заполняет шаблон ``HelloBundle:Hello:index.twig.html``. Но что оно
значит? ``HelloBundle`` это имя бандла, ``Hello`` это контроллер, а ``index.twig.html``
