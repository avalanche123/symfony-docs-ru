...
<<<<<<< .merge_file_iemoEQ
    `документацию <documentation>`_. Этот раздел лишь кратко описывает основные концепты.
=======
...
макет сам по себе был декорирован оным. Когда бандл в имени шаблона не указан
(``::base.twig.html``), то виды ищутся в папке ``app/views/``. Эта папка хранит
глобальные виды для всего проекта:
...
.. tip::
    ...
    Функция ``url`` создает *абсолютные* URL-ы:
