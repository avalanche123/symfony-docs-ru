...
Symfony2  использует возможности `Swiftmailer`_ для рассылки e-mail.

Установка
---------

Включите ``SwiftmailerBundle`` в своем ядре::
...
Единственным обязательным параметром является ``transport``:
...
Допустимы следующие атрибуты настройки:
...
Доступ к Mailer можно получить с помощью сервиса ``mailer`` из действия::
...
.. note::
    Чтобы не усложнять картину, тело email хранится в шаблоне, рендеренном методом ``renderView()``.

Использование Gmail
-----------

Если вы хотите использовать свой аккаунт в Gmail для рассылки писем, используйте специальный транспорт ``gmail``:
